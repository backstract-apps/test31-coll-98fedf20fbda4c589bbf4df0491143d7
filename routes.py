...
@router.post('/persons/')
async def post_persons( file: UploadFile , db: Session = Depends(get_db)):
    return await service.post_persons(db , file)

@router.put('/persons/rollnumber/')
async def put_persons_rollnumber( rollnumber: int, fullname: str, age: str, profession: str , db: Session = Depends(get_db)):
